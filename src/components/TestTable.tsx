...
const AdsTable = ({ productData }: { productData?: any[] }) => {
  // States
  ...
  const columns = useMemo<ColumnDef<any, any>[]>(
    () => [
      ...
      columnHelper.accessor('dailyBudget', {
        header: 'Daily Budget',
        cell: ({ row }) => <Typography>{row.original.dailyBudget}</Typography>
      }),
      columnHelper.accessor('timezone', {
        header: 'Timezone',
        cell: ({ row }) => <Typography>{row.original.timezone}</Typography>
      }),
      ...
      columnHelper.accessor('updatedAt', {
        header: 'Updated At',
        cell: ({ row }) => <Typography>{new Date(row.original.updatedAt).toLocaleString()}</Typography>
      }),
      columnHelper.accessor('totalCampaigns', {
        header: 'Total Campaigns',
        cell: ({ row }) => <Typography>{row.original.totalCampaigns}</Typography>
      }),
